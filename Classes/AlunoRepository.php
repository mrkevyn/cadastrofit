...
class AlunoRepository {
    ...
    public function getDatasTestesFisicos($alunoId){
        $stmt = $this->db->prepare("SELECT DISTINCT DATE(created_at) AS data FROM testes_fisicos WHERE aluno_id = ? ORDER BY data DESC");
        $stmt->execute([$alunoId]);
        return $stmt->fetchALL(PDO::FETCH_COLUMN);
    }
    ...
    public function getCalculos($alunoId, $dataEscolhida) {
        ...
        $sql = "SELECT * FROM composicao_corporal WHERE id_aluno = ? AND DATE(created_at) = ?";
        ...
        $sql = "SELECT * FROM composicao_corporal WHERE id_aluno = ? AND DATE(created_at) > ? ORDER BY DATE(created_at) ASC LIMIT 1";
        $params = [$alunoId, $dataEscolhida];
        $stmt = $this->db->prepare($sql);
    ...
    public function getAllCalculos($id_aluno) {
        $stmt = $this->db->prepare("SELECT * FROM composicao_corporal WHERE id_aluno = ?");
        $stmt->execute([$id_aluno]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
}
